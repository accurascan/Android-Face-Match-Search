...
abstract class FaceLockHelper {
     ...
     * @return result
     */
    native int InitEngine(FaceCallback callback, int fmin, int fmax, float resizeRate, String modelpath, String weightpath, AssetManager assets);
     ...
     */
    native void DetectLeftFace(byte[] vBmp, int width, int height);

    native void DetectRightFace(byte[] vBmp, int width, int height, float[] feature);

    native int DetectRightFaceFeatures(byte[] vBmp, int width, int height, float[] feature, FaceDetectionResult result);
     ...
     */
    native void Extractfeatures(byte[] vBmp, int width, int height, float[] plandmarks, int imgType);

    /**
     * This is the function to calculate the similarity of 2 face feature vectors.
     */
    native float Similarity(float[] vFeat1, float[] vFeat2, int length);

    native float MatchFace(float[] vFeat1, float[][] vFeat2, int length, int[] indexed);
